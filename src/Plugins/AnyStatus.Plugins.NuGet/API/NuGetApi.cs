...
internal class NuGetApi
{
    ...
    internal async Task<NuGetMetadata> GetPackageMetadataAsync(string            packageId
                                                             , bool              prerelease
                                                             , CancellationToken cancellationToken)
    {
        var resource = await GetResourceAsync("SearchQueryService", cancellationToken).ConfigureAwait(false);

        var packagesMetadata = await GetPackagesMetadataAsync(resource
                                                            , packageId
                                                            , prerelease
                                                            , cancellationToken)
        ...
        return packageMetadata ?? throw new ("NuGet package not found.");
    }
    ...
    internal async Task<IEnumerable<NuGetResource>> GetResourcesAsync(CancellationToken cancellationToken)
    {
        ...
        {
            throw new ("An error occurred while getting NuGet resources.");
        }

        return response.Data.Resources;
    }

    internal async Task<IEnumerable<NuGetMetadata>> GetPackagesMetadataAsync(NuGetResource     searchQueryService
                                                                           , string            packageId
                                                                           , bool              prerelease
                                                                           , CancellationToken cancellationToken)
    {
        ...
        if (prerelease)
        {
            metadataRequest.AddParameter("prerelease", true);
        }
        ...
        {
            throw new ($"An error occurred while getting NuGet package metadata. Response: {metadataResponse.StatusDescription}");
        }
