...
internal class JenkinsApi
{
        ...
        where T : new()
    {
        ...
        throw new ("An error response received from Jenkins server.", response.ErrorException);
    }
...
public static class ApiExtensions
{
    public static string GetUrl(this IRestRequest request)
    {
        ...
        for (var i = 0; i < request.Parameters.Count; i++)
        {
            ...
            if (p.Type is ParameterType.GetOrPost or ParameterType.QueryString or ParameterType.UrlSegment)
            {
