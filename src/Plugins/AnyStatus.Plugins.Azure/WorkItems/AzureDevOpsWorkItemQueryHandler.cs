...
public class AzureDevOpsWorkItemQueryHandler : AsyncStatusCheck<AzureDevOpsWorkItemQueryWidget>, IEndpointHandler<IAzureDevOpsEndpoint>
{
    ...
    private static void Sync(TextWidget                     parent
                           , IEnumerable<WorkItemReference> references
                           , CollectionResponse<WorkItem>   workItemsResponse)
    {
        parent.Text = workItemsResponse.Count.ToString();

        foreach (var removedWorkItem in parent.OfType<AzureDevOpsWorkItemWidget>()
                                              .Where(widget => references.All(reference => reference.Id != widget.WorkItemId))
                                              .ToList())
        {
    ...
    {
        var workItemWidget = parent.OfType<AzureDevOpsWorkItemWidget>()
                                   .FirstOrDefault(child => child.WorkItemId.Equals(workItem.Id));
