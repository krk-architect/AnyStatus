...
internal class WindowHandler : RequestHandler<MaterialWindow>
{
    ...
    protected override void Handle(MaterialWindow request)
    {
        ...
        if (content is null)
        {
            throw new ($"An error occurred while creating window content type '{request.Type}'.");
        }
    ...
    private void OnWindowClosed(object sender, EventArgs e)
    {
        ...
        if (window.Content is not IDisposable content)
        {
            return;
        }
    ...
    public static bool IsOffScreen(Window window) => window.Left <= SystemParameters.VirtualScreenLeft - window.Width
                                                  ...
                                                  || window.Top  >  SystemParameters.VirtualScreenTop  + SystemParameters.VirtualScreenHeight;
}