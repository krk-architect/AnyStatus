...
internal class PropertyViewModelBuilder : IPropertyViewModelBuilder
{
    ...
    public PropertyViewModelBuilder(PropertyViewModelFactory propertyViewModelFactory)
    {
    ...
    public IEnumerable<IPropertyViewModel> Build(object source)
    {
        ...
        viewModels.AddRange(GetProperties(source)
                           .Select(property => Build(source, property, viewModels))
    ...
    private IPropertyViewModel Build(object source, PropertyInfo propertyInfo, IEnumerable<IPropertyViewModel> properties)
    {
        ...
        property.Header = displayNameAttribute is null || string.IsNullOrWhiteSpace(displayNameAttribute.DisplayName)
                              ? propertyInfo.Name
    ...
    private static IEnumerable<PropertyInfo> GetProperties(object source) => source.GetType()
                                                                                   .GetProperties()
                                                                                   .Where(p => p.CanWrite && p.IsBrowsable())
                                                                                   .OrderBy(p => p.Order());
}