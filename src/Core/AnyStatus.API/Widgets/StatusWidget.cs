...
internal interface ICheckStatusOf<TStatusWidget> : IRequestHandler<StatusRequest<TStatusWidget>>
    where TStatusWidget : IStatusWidget { }

public class StatusRequest<TStatusWidget> : Request<TStatusWidget>
    where TStatusWidget : IStatusWidget
{
...
{
    public static StatusRequest<T> Create<T>(T context)
        where T : IStatusWidget => new (context);
}
