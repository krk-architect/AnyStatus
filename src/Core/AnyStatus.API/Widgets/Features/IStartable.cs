...
public interface IStart<T> : IRequestHandler<StartRequest<T>>
    where T : IStartable { }

public static class StartRequestFactory
{
    public static StartRequest<T> Create<T>(T context)
        where T : IStartable => new (context);
}

public class StartRequest<T> : Request<T>
    where T : IStartable
{
