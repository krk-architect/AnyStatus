...
public sealed class EndpointHandlerDecorator<TRequest, TResponse, TContext, TEndpoint>
    ...
    where TEndpoint : IEndpoint
{
    ...
    public Task<TResponse> Handle(TRequest request, CancellationToken cancellationToken)
    {
        if (_handler is IEndpointHandler<TEndpoint> endpointHandler)
        {
            ...
            endpointHandler.Endpoint = _context.Endpoints
                                               .OfType<TEndpoint>()
                                               .FirstOrDefault(endpoint => endpoint.Id == request.Context.EndpointId);
