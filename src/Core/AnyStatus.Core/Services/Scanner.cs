...
public class Scanner : IScanner
{
    ...
    public static IEnumerable<Type> GetTypesOf(Type type, bool browsableOnly = true)
        => from assembly in GetAssemblies()
           from t in assembly.GetTypes()
           where t.IsClass && !t.IsAbstract && type.IsAssignableFrom(t) && (!browsableOnly || t.IsBrowsable())
           select t;

    public static IEnumerable<Type> GetTypesOf<T>(bool browsableOnly = true)
        => GetTypesOf(typeof(T), browsableOnly);

    public static IEnumerable<Category> GetWidgetCategories()
        => from type in GetTypesOf<IWidget>()
