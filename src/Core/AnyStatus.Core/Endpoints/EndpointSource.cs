...
public class EndpointSource : IEndpointSource
{
    ...
    private static Type GetRequiredEndpoint(object source)
        => source.GetType()
                 .GetInterfaces()
                 .FirstOrDefault(static i => i.Name == typeof(IRequireEndpoint<>).Name)
                ?.GetGenericArguments()
                 .FirstOrDefault()
    ...
    private List<NameValueItem> GetEndpointsAssignableFrom(Type requiredEndpoint)
        => _context.Endpoints?
                   .Where(requiredEndpoint.IsInstanceOfType)
                   .Select(static endpoint => new NameValueItem(endpoint.Name, endpoint.Id))
                   .ToList();
}